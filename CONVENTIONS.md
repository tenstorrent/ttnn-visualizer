...
- **Key every registry by the attribute name.** `_ENV_PARSERS`, `_STRICT_BOOLEANS`, `_ENV_ALIASES`, `_ENV_OVERRIDE_SKIP` and `_ENV_NAME_UNREAD` are all keyed by attribute; the variable is resolved through `_env_name_for` at lookup time and used only in messages. Keying one by the *variable* would pass the guard tests and never fire for the one setting that has an alias.
