...
**Documented exception.** The unload flush in `src/functions/recordEvent.ts` calls `navigator.sendBeacon` rather than `axiosInstance`, because a beacon is the only request the browser guarantees to send while the document is being discarded. It therefore gets neither interceptor, and composes `BASE_PATH` + `Endpoints.EVENT_LOGGING` by hand — deliberately without `instanceId`, since `POST /api/event-log/events` is deployment/session-scoped and takes no `@with_instance`. The body must be a `Blob` typed `application/json`: the route requires that content type so the request stays non-simple and a hostile origin needs a preflight `ALLOWED_ORIGINS` refuses, and a bare-string beacon is sent as `text/plain` and rejected.
...
`backend/ttnn_visualizer/tests/test_event_logging_frontend_parity.py` pins more than the enums: the detail fields each client event declares, the route path, the `{ events }` envelope key, and that a full batch of the largest event still fits `MAX_EVENT_LOG_REQUEST_BYTES`. `backend/ttnn_visualizer/tests/test_event_logging_docs_parity.py` separately pins the structured event, field and enum reference in `docs/src/event-logging.md`.

The event list exists to answer Q1–Q5 in #1819, not to accumulate counters. Every proposed event must name the decision it informs. **Adding or changing an event, detail field or closed value means updating `event_logging.py`, the client copy where applicable, and the user reference in one commit.** The two parity tests are the review gate: a code-only vocabulary change, or prose that omits part of the bounded schema, must fail CI.
...
|---|---|---|
| `MAX_BUFFERED_EVENTS` | `src/functions/recordEvent.ts` | Events the client holds, and the size of a batch it builds |
| `MAX_EVENT_LOG_BATCH_EVENTS` | `backend/ttnn_visualizer/event_logging.py` | Lines one `os.write` in `_append_line` must hold — **write atomicity**, not an HTTP body |
| `MAX_EVENT_LOG_REQUEST_BYTES` | `backend/ttnn_visualizer/views.py` | Bytes one page may post per request (`request.max_content_length`) |

The first two **must be equal**, so a batch the client builds can never be refused wholesale; the third must fit a full batch of the largest event. The batch cap lives in `event_logging.py`, beside the atomicity guarantee it bounds — raising it in `views.py` alone would silently relax that guarantee, and `_write_events` enforces it so a second caller can't bypass the route.
...
**Don't** pass the JSON string directly — this is the one place a content type is a trust boundary rather than a formality (see [CORS is a trust boundary](#cors-is-a-trust-boundary-not-deployment-plumbing)). The beacon is also a documented exception to the [`axiosInstance` rule](#use-axiosinstance-for-http-requests-never-call-raw-axiosgetpostputdelete-at-a-call-site), composing `BASE_PATH` + `Endpoints.EVENT_LOGGING` by hand the way axios's `combineURLs` does so a non-root `BASE_PATH` still resolves.
...
### Document a new tool in the same change that registers it

`docs/src/agent-tools.md` is the only description of this surface an agent's reader ever gets. There is no UI beside it, no schema browser, and nothing that lists the tools at a glance — a client picks from that table and reads the prose under it to know how to interpret what comes back.
...
Two things make this sharper than ordinary documentation drift. #2035 puts that page inside the application, at which point a missing row is a hole a user sees rather than a stale file in a repository. And the surface has drifted here before: #2034 is a tool whose own docstring describes behaviour it does not have.
...
- **Key every registry by the attribute name.** `_ENV_PARSERS`, `_STRICT_BOOLEANS`, `_ENV_ALIASES`, `_ENV_OVERRIDE_SKIP` and `_ENV_NAME_UNREAD` are all keyed by attribute. For every one but `_ENV_NAME_UNREAD` the variable is then resolved through `_env_name_for` at lookup time and used only in messages; `_ENV_NAME_UNREAD` reads the attribute name directly, because a setting listed there has no alias by definition — see [the descriptor rules below](#the-loop-walks-the-mro-and-skips-derived-settings). Keying one by the *variable* would pass the guard tests and never fire for the one setting that has an alias.
- **Keys are strings, so nothing binds them to the attribute.** A rename leaves a dead entry that silently falls back to type dispatch — or, in `_STRICT_BOOLEANS`, downgrades a refusal to a warning; or, in `_ENV_OVERRIDE_SKIP`, starts accepting env strings for a derived attribute. `test_every_env_parser_names_a_real_setting`, `test_every_strict_boolean_names_a_real_boolean_setting`, `test_every_env_override_skip_names_a_real_setting`, `test_every_unread_name_is_a_descriptor_backed_setting` and `test_every_registry_is_keyed_the_way_it_is_looked_up` are what catch it.

#### `_ENV_ALIASES` where the variable isn't named after the attribute

Every setting that reads a variable at all reads one named after its attribute, which is right for all of them but `DEBUG`: it is fed by `FLASK_DEBUG`, because a bare `DEBUG` is the log-level knob (see the entry under [Known inconsistencies](#known-inconsistencies)). `USAGE_RECORDING_ACTIVE` is the other departure and is not an alias — it reads no variable of its own at all; see [the descriptor rules below](#the-loop-walks-the-mro-and-skips-derived-settings). `_ENV_ALIASES` maps the attribute to the variable that owns it and `_env_name_for` is the single reader, so the aliased spelling is written once. Without it, `DEBUG=true` — what `pnpm flask:start-debug` sets — turned on Flask's debug mode under `FLASK_ENV=production`, which suppresses the catch-all error handler and returns tracebacks. Add an entry here rather than a special case in the loop if another setting ever grows a differently-named variable.
...
**`USAGE_RECORDING_ACTIVE` is named for the state, not for a variable, and that is settled** (#1921). It is fed by `USAGE_RECORDING_DISABLED` — the opposite polarity — and the attribute is what `PRINT_ENV` and `to_dict` publish, so borrowing the variable's name would publish `true` when recording is off. Don't "fix" the asymmetry by renaming the attribute or by adding a positive-polarity variable: a second opt-out spelling has to be documented in `.env.sample`, honoured in `_is_recording_disabled_by_environment`, and named correctly by the launch banner, forever, to remove an inconsistency that costs one paragraph.

The two descriptors are skipped for the same reason but are not equivalent, and **`_ENV_NAME_UNREAD` is the registry that records which is which**. `ALLOWED_ORIGINS` reads `os.getenv("ALLOWED_ORIGINS")` inside its own `__get__`, so a value set for it is honoured and the silence is right. `USAGE_RECORDING_ACTIVE`'s own name reaches nothing, so `_report_unread_name` warns — the skip branch runs *before* the loop reads the environment, so `_report_ignored_skip` can never cover it. That warning is what pays for the naming decision above: `PRINT_ENV` prints the attribute as `KEY=value` beside settings that really are variables, so copying `USAGE_RECORDING_ACTIVE=false` back out is the obvious mistake to make, and this is the privacy opt-out — failing silently means recording against someone who believes they switched it off. Set in the shell, it is caught in the launching process. Set in a `.env`, it is caught only in the workers: `main()` builds the `Config()` singleton through `_config_after_cli_env` *before* `create_app()` calls `load_dotenv`, so the launcher itself still ignores the value in silence and `PRINT_ENV` still prints it unflagged. Each gunicorn worker re-runs `create_app()` in a fresh process and does warn — but not visibly under `--daemon`. Widening that is the `load_dotenv`-before-`Config()` ordering problem, not this registry's. **The registry's value is a remedy callable, not a variable name, because a name alone cannot carry the polarity:** "set `USAGE_RECORDING_DISABLED` instead" reads as a rename, and an operator carrying their `false` across lands on `USAGE_RECORDING_DISABLED=false` — a recognised boolean that leaves recording on and warns about nothing, turning one loud failure into a loud step followed by a silent one. So the advice depends on the value set, and both directions come from `event_logging.py`: a recognised `true` gets `describe_opt_in(server_mode)`, anything else gets `describe_opt_out(server_mode)` — which owns that sentence for the launch banner too. **Neither may assert the current state.** The loop does not consult the marker file, and a `settings_override` posture never reaches the environment, so any sentence describing the state is wrong for somebody. The helpers therefore state only that recording defaults on and name the environment-selected posture's opt-outs. Keyed by attribute like every other registry, `_ENV_NAME_UNREAD` takes the attribute name directly rather than through `_env_name_for`, because a setting listed here has no alias by definition. `test_the_descriptor_inventory_is_pinned` forces a new descriptor into one camp or the other; the ordinary inventory test excludes descriptors by construction, which is how this went unnoticed.

**The warning is the whole remedy: this deliberately fails open.** `USAGE_RECORDING_ACTIVE=false` is reported and then ignored — recording continues. That is a real residual gap, and it is worth stating why rather than rediscovering it: `_is_recording_disabled_by_environment` *obeys* an unrecognised value for its own variable, on the reasoning that the cost of honouring a typo is one missing data point and the cost of ignoring it is recording against an explicit request, so the opposite choice here is a genuine asymmetry. The asymmetry is the point. Honouring `USAGE_RECORDING_ACTIVE=false` would make it a second working opt-out spelling, which is what #1921 settled against — it would need honouring in `_is_recording_disabled_by_environment`, keeping in step with the launch banner, and an answer for why `PRINT_ENV` publishes a name that is half a control. The accepted cost is the operator who never sees stderr — a daemonised gunicorn, a container whose stdout is dropped — who stays recorded after making exactly this mistake. Revisit if that operator turns up; don't quietly "fix" it into an opt-out on the strength of the neighbouring function alone.

The remedy sentences are advice, so they must not be able to *stop* the app: both resolve the marker path, and `Path.home()` raises `RuntimeError` when HOME is unset and the uid is absent from passwd. `_nameable_marker_path` swallows that and the marker clause drops, which is also the truthful reading — with no resolvable home there is nowhere for the marker to live, so the variable really is the only control left. Building the advice eagerly inside the override loop had turned a variable that configures nothing into a failure to start for an arbitrary-uid container with `APP_DATA_DIRECTORY` set (`test_the_advice_survives_a_home_directory_that_cannot_be_resolved`).
