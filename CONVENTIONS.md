...
- **`USAGE_RECORDING_ACTIVE` is a config attribute with no matching variable** — it is fed by `USAGE_RECORDING_DISABLED`, the opposite polarity, so it is named for the state instead. Borrowing the variable's name would make `PRINT_ENV` publish `true` when recording is off. **Settled: the name stays.** Its one cost is that `PRINT_ENV` prints it as `KEY=value` beside settings that *are* variables, so `USAGE_RECORDING_ACTIVE=false` in a `.env` is the obvious mistake to make — and this is the privacy opt-out, so failing silently means recording against someone who believes they switched it off. `_ENV_NAME_UNREAD` makes the loop warn instead of ignoring it without a word, and the warning names the opt-out *at its value* — naming the variable alone would send the operator to `USAGE_RECORDING_DISABLED=false`, which is silently a no-op. (#1921)
