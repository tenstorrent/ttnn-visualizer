...
2. Install documentation dependencies with `uv sync --extra docs` (or `pip install ".[docs]"` in a Python virtual environment).
3. Run `uv run sphinx-build docs docs/output` (or navigate to `./docs` and run `sphinx-build . output`).
