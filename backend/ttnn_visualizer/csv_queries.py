...
from ttnn_visualizer.models import TabSession, RemoteConnection
...
class LocalCSVQueryRunner:
    ...
    def execute_query(
        self, columns: List[str], filters: Dict[str, str] = None, as_dict: bool = False
    ) -> Union[List[List[str]], List[Dict[str, str]]]:
        """
        Executes a query on the loaded DataFrame.
        ...
        # Rename columns to remove spaces if as_dict is True
        if as_dict:
            sanitized_columns = {
...
class RemoteCSVQueryRunner:
    ...
    def execute_query(
        ...
        """
        Execute a query to fetch specific columns with optional filters.
        ...
        awk_cmd = (
            f"awk -F'{self.sep}' 'NR > {self.offset + 1} && {awk_filter} {{print {awk_columns}}}' {self.file_path}"
            if filters
            else f"awk -F'{self.sep}' 'NR > {self.offset + 1} {{print {awk_columns}}}' {self.file_path}"
        )
...
class DeviceLogProfilerQueries:
    ...
    def query_zone_statistics(
        self, zone_name: str, as_dict: bool = False
    ) -> Union[List[List[str]], List[Dict[str, str]]]:
        """
...
if __name__ == "__main__":
    ...
    # Local Example
    profiler_directory = "/your_local_profiler_directory"
    tab_session = TabSession(tab_id="1234", profiler_directory=profiler_directory)
    device_log_queries = DeviceLogProfilerQueries(session=tab_session)

    with DeviceLogProfilerQueries(session=tab_session) as queries:
        print(queries.query_zone_statistics(zone_name="BRISC-FW", as_dict=True))

    # Remote Example
    connection = RemoteConnection(
        name="Test",
        username="your_username",
        port=22,  # change to your port
        path="/report/path",
        host="your_host",
    )

    remote_profiler_directory = "/your_remote_profiler_directory"

    tab_session = TabSession(
        tab_id="1234",
        profiler_directory=remote_profiler_directory,
