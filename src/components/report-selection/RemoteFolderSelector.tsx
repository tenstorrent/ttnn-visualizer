                    ...
                    labelElement={
                        showLinkStatus ? (
                            <FolderLinkStatusIcon
                                linkState={getFolderLinkState(remotePath, linkedPaths, unlinkedPaths)}
                            />
                        ) : undefined
                    }
                />
...
const RemoteFolderSelector = ({
    ...
    // Linked first, unknown next, failed links last — preserve existing order within each group.
    const sortedFolderList = useMemo(() => {
        if (!linkedPaths?.size && !unlinkedPaths?.size) {
            return remoteFolderList;
        }

        return [...remoteFolderList].sort((a, b) =>
            compareByFolderLinkState(a.remotePath, b.remotePath, linkedPaths, unlinkedPaths),
        );
    }, [remoteFolderList, linkedPaths, unlinkedPaths]);
        ...
        <div className='form-container'>
            ...
            >
                <Button
                    ...
                    endIcon={!loading && remoteFolderList?.length > 0 ? IconNames.CARET_DOWN : undefined}
                    disabled={isDisabled}
                    loading={loading}
                    text={remoteFolder?.reportName ?? fallbackLabel}
                    title={loading ? 'Activating report…' : undefined}
