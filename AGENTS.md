...
- **Adding a setting means classifying it twice**, and a test fails until you do each: into `_OVERRIDABLE_SETTINGS` or one of the skip sets, and — if overridable — as pinned or inherited for the test fixtures. See #1869. A **descriptor-backed** setting is classified a third time instead of the second, into `_ENV_NAME_UNREAD` (its own name configures nothing, so setting it warns) or the test-local `_SELF_READING_DESCRIPTORS` (it reads its own variable, so silence is correct); `test_the_descriptor_inventory_is_pinned` fails until you do. See #1921.
