...
- **A derived value read by several hooks *and* by one component instance per virtualised row goes through `memoiseLatest`, not `useMemo`.** `useMemo` caches per invocation, so an O(rows) derivation reached once per row runs once per row. Row fanout is what makes the trade pay — a value with a couple of consumers and no per-row reader stays on `useMemo` (`usePerformanceRange`, `useL1PressureByOperation`), because a cache of one degrades to no cache when two live callers pass different arguments. Only reach for `memoiseLatest` when every argument comes from a source all consumers share (a query result, not a per-caller `useMemo`). Results are shared, so treat them as immutable, and pair any `queryClient.clear()` with the matching cache reset (`clearReportCaches`).
...
`src/functions/recordUsage.ts` posts local usage events to `POST /api/usage`. Four invariants a reader cannot infer from the code alone:

- **`usage.py` owns the vocabulary; `src/definitions/UsageEvent.ts` is a copy.** Validation is server-side because a client cannot enforce it. Nothing at runtime reports a divergence — the endpoint answers 422 and the client swallows it — so `backend/ttnn_visualizer/tests/test_usage_frontend_parity.py` is the only thing that notices. It also pins the route path, the `{ events }` envelope key, and that every client-facing enum has a copy. Change both sides in one commit.
- **`MAX_BUFFERED_EVENTS` must equal `MAX_USAGE_BATCH_EVENTS`**, which bounds write atomicity in the writer rather than merely an HTTP body. Pinned by the same test.
- **The unload beacon must send a `Blob` typed `application/json`.** That content type is what keeps the request non-simple; a bare-string beacon goes as `text/plain` and the route refuses it.
- **Failures are silent, and batches are never re-buffered.** The endpoint answers 204 whether it wrote or recording is off, so there is nothing to branch on. The only diagnostic is a `console.warn` under `import.meta.env.DEV`, status only — never a response body.
...
- Env-var booleans go through **`_parse_env_bool("FOO", False)`** in `settings.py`, or **`str_to_bool`** elsewhere (query params, `devtools/`) — never `bool(os.getenv(...))` (truthy for `"false"`). **`parse_bool`** owns the accepted vocabulary — only `true`/`1`/`false`/`0`, matching the SPA's `isFlagEnabled` (which backs `isServerModeEnabled` and `USAGE_RECORDING_ACTIVE`) — and returns `None` for a typo; never re-declare those tokens at a call site, and treat widening or narrowing the set as a change to both sides and to the HTTP query params. One setting deliberately **obeys** the typo rather than discarding it: `_is_recording_disabled_by_environment` (`usage.py`) switches recording off for any `USAGE_RECORDING_DISABLED` value that isn't a recognised false, because reading a typo as consent to record is the wrong way to be wrong for a privacy opt-out — don't "fix" it back to `str_to_bool` ([CONVENTIONS.md](./CONVENTIONS.md#config-reports-a-typo-rather-than-obeying-it-and-_strict_booleans-refuses-to-start)). Three traps are worth knowing before you touch `settings.py`, all covered in [CONVENTIONS.md](./CONVENTIONS.md#env-var-booleans-go-through-_parse_env_bool--str_to_bool): a setting in **`_STRICT_BOOLEANS`** (`SERVER_MODE`) *refuses to start* rather than guess; `DEBUG` is fed by **`FLASK_DEBUG`** via `_ENV_ALIASES`, and is forced off under `SERVER_MODE`; and `override_with_env_variables` walks the MRO but skips the attributes named in **`_ENV_OVERRIDE_SKIP`** (the union of `_ENV_OVERRIDE_DERIVED` / `_ENV_OVERRIDE_CONSTANTS` / `_ENV_OVERRIDE_UNCONFIGURED`), then calls **`recompute_derived_settings()`** to rebuild the derived group — don't assume a skipped setting is late-overridable, don't put a guard in the loop expecting it to cover one, and add new derivations to `recompute_derived_settings` rather than at a call site. Adding a setting means classifying it twice, and a test fails until you do each: in `_OVERRIDABLE_SETTINGS` or one of the skip sets (`test_the_settings_inventory_is_pinned`), and — for an overridable one — as pinned or inherited for the test fixtures (`test_the_test_fixtures_pin_every_env_reachable_setting`). The second exists because `TT_METAL_HOME` is exported on any machine that profiles TT-Metal, so an unpinned fixture tests the developer's environment rather than the app; see #1869.
