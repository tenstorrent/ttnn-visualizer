...
- **Derived values shared by many hooks go through `memoiseLatest`, not `useMemo`.** `useMemo` caches per invocation, so an O(rows) derivation read by one component per virtualised row runs once per row. Only use it when every argument comes from a source all consumers share (a query result, not a per-caller `useMemo`) — a cache of one degrades to no cache when two live callers pass different arguments. Results are shared, so treat them as immutable, and pair any `queryClient.clear()` with the matching cache reset (`clearReportCaches`).
