...
- Env-var booleans go through **`_parse_env_bool("FOO", False)`** in `settings.py`, or **`str_to_bool`** elsewhere — never `bool(os.getenv(...))` (truthy for `"false"`). **`parse_bool`** owns the accepted vocabulary — only `true`/`1`/`false`/`0`, matching the SPA's `isServerModeEnabled`, so widening one side means widening the other — and returns `None` for a typo; never re-declare those tokens at a call site. `override_with_env_variables` coerces each value back to its declared type (`_coerce_env_value`), with richer parses registered as named functions in **`_ENV_PARSERS`** that the class body calls too; anything it can't represent is logged and discarded. The loop only reaches the *concrete* config class, so don't assume an environment variable changed after import takes effect, and don't put a guard there expecting it to cover a setting ([#1857](https://github.com/tenstorrent/ttnn-visualizer/issues/1857)).
