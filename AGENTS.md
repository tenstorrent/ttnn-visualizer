...
- Env-var booleans go through **`_parse_env_bool("FOO", False)`** in `settings.py`, or **`str_to_bool`** elsewhere (including `devtools/`) — never `bool(os.getenv(...))` (truthy for `"false"`). **`parse_bool`** owns the accepted vocabulary — only `true`/`1`/`false`/`0`, matching the SPA's `isServerModeEnabled`, so widening one side means widening the other — and returns `None` for a typo; never re-declare those tokens at a call site. `SERVER_MODE` passes **`strict=True`** and so *raises* on an unreadable value: its fallback is the local posture, so guessing would un-gate `@local_only`, and the warning would land on `stderr` before logging is configured. Other booleans warn and keep their default. `override_with_env_variables` coerces each value back to its declared type (`_coerce_env_value`), with richer parses registered as named functions in **`_ENV_PARSERS`** that the class body calls too; anything it can't represent is logged and discarded. It reads a variable named after the attribute unless **`_ENV_ALIASES`** says otherwise — `DEBUG` is fed by `FLASK_DEBUG`, since a bare `DEBUG` is the log-level knob and must not switch on Flask's debug mode. The loop only reaches the *concrete* config class, so don't assume an environment variable changed after import takes effect, and don't put a guard there expecting it to cover a setting ([#1857](https://github.com/tenstorrent/ttnn-visualizer/issues/1857)).
