...
- **Adding a tool touches two places on purpose:** `_tool_table` in `server.py`, and the tool table in [docs/src/agent-tools.md](./docs/src/agent-tools.md) in the same change. That page is not a description of the surface, it is the only one an agent's reader ever gets — a tool registered but undocumented is invisible to the client that would have chosen it, and a caveat that never reaches the page is one an agent acts without. Where a tool's answer needs a caveat to be read correctly, the prose beside the table is where it goes. See #2036.
...
- **`src/definitions/`** holds *primitives*: enums, route/endpoint maps, plot/colour configs, plain interfaces. **`src/model/`** holds richer domain types — usually API response shapes, sometimes classes with methods. If it mirrors a backend response, it's a model. The split applies everywhere; a few modules keep the same basename on both sides (`PerfTable.ts`, `RemoteConnection.ts`, `MlirServer.ts`, `ReportLinks.ts`) — the config half in `definitions/`, the record half in `model/`.
...
- Backend module-private helpers prefix with a single underscore (e.g. `_file_path_from_stack_source_request`), **including helpers in test modules**. Pytest fixtures are the exception — pytest resolves them by name from the test signature, so they stay unprefixed, as do `unittest` lifecycle methods (`setUp`, `tearDownClass`).
...
- **`settings.py` has traps worth reading before you touch it** — all of them in [CONVENTIONS.md](./CONVENTIONS.md#env-var-booleans-go-through-_parse_env_bool--str_to_bool). In short: `SERVER_MODE` is in `_STRICT_BOOLEANS` and *refuses to start* on a value it can't read; `DEBUG` is fed by `FLASK_DEBUG` via `_ENV_ALIASES` and forced off under `SERVER_MODE`; `override_with_env_variables` skips everything in `_ENV_OVERRIDE_SKIP`, so put new derivations in `recompute_derived_settings()`, not at a call site; and `USAGE_RECORDING_DISABLED` is an opt-out where unset/`false`/`0` records, `true`/`1` disables, and an unrecognised value disables with an adjacent startup warning (`_record_launch`, `app.py`) — don't "fix" it back to `str_to_bool`.
- **Adding a setting means classifying it twice**, and a test fails until you do each: into `_OVERRIDABLE_SETTINGS` or one of the skip sets, and — if overridable — as pinned or inherited for the test fixtures. See #1869. A **descriptor-backed** setting is classified *once instead of either of those*, into `_ENV_NAME_UNREAD` (its own name configures nothing, so setting it warns) or the test-local `_SELF_READING_DESCRIPTORS` (it reads its own variable, so silence is correct); `test_the_descriptor_inventory_is_pinned` fails until you do. Don't also list it in `_OVERRIDABLE_SETTINGS` or a skip set — `test_the_settings_inventory_is_pinned` builds its expectation from the non-descriptors and asserts set equality, so a descriptor named there fails it. See #1921.
