...
4. Enter or adjust SSH connection details (hostname or Host alias, username, and report paths). The app always connects as `user@host` and passes the form’s port explicitly, so both override any `User` or `Port` in SSH config (including `Host *`). That can differ from bare `ssh my-alias` in a terminal when the stanza has no `User`/`Port` and the form uses a different default. Editing any field the test exercises — host, username, port, identity file, or either report path — discards an earlier test result; renaming the connection does not.
